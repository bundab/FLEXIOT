...
public class RegisterDevice {
    ...
    // Registry Server
    private final String regAasUrl;
    private final JSONObject regAasJson;

    private final String regStaticSmUrl;
    private final JSONObject regStaticSmJson;
    private final String regDynamicSmUrl;
    private final JSONObject regDynamicSmJson;

    //AAS Server
    private final String aasAasUrl;
    private final JSONObject aasAasJson;

    private final String aasStaticSmUrl;
    private final JSONObject aasStaticSmJson;
    private final String aasDynamicSmUrl;
    private final JSONObject aasDynamicSmJson;
    ...
    public RegisterDevice(String id, DeviceType dt, String username) {
        this.ID = id;
        this.dt = dt;
        this.username = username;

        // AAS Registry Server RegisterDevice Variables
        ...
        for (String element : this.staticElements) {
            this.staticElementsUrls.add(new GenerateUrl().generateUrl(this.ID, StaticSubmodelElementType.valueOf(element)));
        }

        for (String element : this.dynamicElements) {
            this.dynamicElementsUrls.add(new GenerateUrl().generateUrl(this.ID, DynamicSubmodelElementType.valueOf(element)));
        }
        ...
        for (String staticElement : this.staticElements) {
            if (Objects.equals(staticElement, StaticSubmodelElementType.Owner.toString())) {
                this.staticElementsJsons.add(new GenerateJson().generateJson(this.ID, this.dt, StaticSubmodelElementType.valueOf(staticElement), this.username));
            } else {
                this.staticElementsJsons.add(new GenerateJson().generateJson(this.ID, this.dt, StaticSubmodelElementType.valueOf(staticElement)));
            }
        }

        for (String dynamicElement : this.dynamicElements) {
            // Choose between String and Float type values
            if (new GenerateJson().generateJson(this.ID, this.dt, DynamicSubmodelElementType.valueOf(dynamicElement), "").toString().contains("\"valueType\": \"float\",")) {
                this.dynamicElementsJsons.add(new GenerateJson().generateJson(this.ID, this.dt, DynamicSubmodelElementType.valueOf(dynamicElement), 0));
            } else {
                this.dynamicElementsJsons.add(new GenerateJson().generateJson(this.ID, this.dt, DynamicSubmodelElementType.valueOf(dynamicElement), ""));
            }
        }
    }

    public void execute() throws Exception {
        // The client side will send the topics precisely during the registration process with MQTT publishing (Add/Delete Topic for the Manager)!!!
