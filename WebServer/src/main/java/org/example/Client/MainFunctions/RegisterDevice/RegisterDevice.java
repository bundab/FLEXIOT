...
public class RegisterDevice {
    ...
    public RegisterDevice(String id, DeviceType dt, String username) {
        ...
        for(int i = 0; i < this.staticElements.size(); i++) {
            if(Objects.equals(staticElements.get(i), StaticSubmodelElementType.Owner.toString())) {
                this.staticElementsJsons.add(new GenerateJson().generateJson(this.ID, this.dt, StaticSubmodelElementType.valueOf(this.staticElements.get(i)), this.username));
            } else {
